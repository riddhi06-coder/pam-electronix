    ...
    public function update(Request $request, $id)
    {
        // dd($request);
          $validatedData = $request->validate([
            ...
            'special_heading' => 'required|string',
            'special_description' => 'required|string',
            'special_image' => 'sometimes|image|mimes:jpg,jpeg,png,webp|max:2048',
        ...
        if ($request->hasFile('product_image')) {
            ...
            $productImageName = time() . rand(1000, 9999) . '.' . $productImage->getClientOriginalExtension();
            $productImage->move(public_path('uploads/product/'), $productImageName);
        }

        $orderingImageName = $product->image;
        if ($request->hasFile('image')) {
            $ordering = $request->file('image');
            $orderingImageName = time() . rand(10000, 99999) . '.' . $ordering->getClientOriginalExtension();
            $ordering->move(public_path('uploads/product/'), $orderingImageName);
        }

        $specialImageName = $product->special_image;
        if ($request->hasFile('special_image')) {
            $special = $request->file('special_image');
            $specialImageName = time() . rand(100000, 999999) . '.' . $special->getClientOriginalExtension();
            $special->move(public_path('uploads/product/'), $specialImageName);
        }

        $printImageNames = json_decode($product->print_image, true) ?? [];
        if ($request->hasFile('print_image')) {
            foreach ($request->file('print_image') as $printImage) {
                if ($printImage) {
                    $filename = time() . rand(1000, 9999) . '.' . $printImage->getClientOriginalExtension();
                    $printImage->move(public_path('uploads/product/'), $filename);
                    $printImageNames[] = $filename;
