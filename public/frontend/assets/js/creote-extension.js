...
function creote_swiper_with_tab() {
    ...
    $(function () {
      // Check window width to apply different settings
      if ($(window).width() > 768) {
        // Initialize freezeTable for desktop (larger screens)
        $('.freeze-table').freezeTable({
          scrollBar: true,
          columnNum: 4, // Freeze the first 4 columns
          columnKeep: true,
          scrollable: true,
          freezeHead: false, // Optional: leave header non-frozen on desktop
        });
      } else {
        // On mobile, disable freezeTable and column freezing
        $('.freeze-table').freezeTable({
          scrollBar: true,
          columnNum: -1, // Disable column freezing on mobile
          columnKeep: false,
          scrollable: true,
          freezeHead: true, // Keep header frozen on mobile for better UX
        });

        // Mobile: Apply sticky header and horizontal scroll
        $('.freeze-table thead th').css({
          position: 'sticky',
          top: 0,
          background: '#b78d52',
          'z-index': 10,
