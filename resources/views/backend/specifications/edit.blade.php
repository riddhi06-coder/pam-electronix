        ...
        <div class="page-body">
          <div class="container-fluid">
            <div class="page-title">
              <div class="row">
                <div class="col-6">
                  <h4>Edit Specification Form</h4>
                </div>
                <div class="col-6">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                    <a href="{{ route('manage-specifications.index') }}">Home</a>
                    </li>
                        ...
                        <div class="row g-3">    
                            ...
                            <div class="tab-content" id="wizard-tabContent">
                                <div class="tab-pane fade show active" id="wizard-contact" role="tabpanel" aria-labelledby="wizard-contact-tab">
                                    <form class="row g-3 needs-validation custom-input" novalidate action="{{ route('manage-specifications.update', $details->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                       ...
                                       <!-- Banner Image -->
                                        <div class="col-md-6">
                                            ...
                                            <!-- Preview Section - below the label -->
                                            <div id="bannerImagePreviewContainer" style="{{ $details->banner_image ? '' : 'display: none;' }} margin-bottom: 10px;">
                                                <img
                                                    id="banner_image_preview"
                                                    src="{{ asset('uploads/specifications/' . $details->banner_image) }}"
                                                    alt="Preview"
                                                    class="img-fluid"
                                        ...
                                        <div class="col-md-12" id="ImagePreviewContainer" style="{{ $details->image ? '' : 'display: none;' }}">
                                            <img
                                                id="image_preview"
                                                src="{{ asset('uploads/specifications/' . $details->image) }}"
                                                alt="Preview"
                                                class="img-fluid"
                                                style="max-height: 200px; border: 1px solid #ddd; padding: 5px;">
                                        </div>
                                        ...
                                        <div class="col-12 mb-5">
                                            ...
                                            <textarea class="form-control summernote" id="summernote" name="detailed_description" placeholder="Enter Description" required>{{ old('detailed_description', $details->detailed_description) }}</textarea>
                                            <div class="invalid-feedback">Please enter a detailed description.</div>
                                        </div><br><br><br><br><br><br>


                                        <!-- Product Headers Table -->
                                        <h4>Specifications Table Headers</h4>
                                        <table class="table table-bordered p-3" id="headerTable" style="border: 2px solid #dee2e6;">
                                            ...
                                            <tbody>
                                                @php
                                                    $headers = json_decode($details->headers ?? '[]', true);
                                                @endphp

                                                @if(!empty($headers))
                                                    @foreach($headers as $index => $header)
                                                    <tr>
                                                        <td><input type="text" class="form-control" name="header[]" value="{{ old('header.' . $index, $header) }}" required></td>
                                                        @if($loop->first)
                                        ...
                                        <table class="table table-bordered p-3" id="specsTable" style="border: 2px solid #dee2e6;">
                                            ...
                                            <tbody>
                                                @php
                                                    $specIds = json_decode($details->spec_ids ?? '[]', true);
                                                    $tempCoeffs = json_decode($details->spec_temp_coeff ?? '[]', true);
                                                    $drifts = json_decode($details->spec_capacitor_drift ?? '[]', true);
                                                    $maxRows = max(count($specIds), count($tempCoeffs), count($drifts));
                                                @endphp

                                                @if($maxRows > 0)
                                                    @for($i = 0; $i < $maxRows; $i++)
                                                    <tr>
                                                        <td><input type="text" class="form-control" name="id[]" value="{{ old('id.' . $i, $specIds[$i] ?? '') }}"></td>
                                                        <td><input type="text" class="form-control" name="temp_coeff[]" value="{{ old('temp_coeff.' . $i, $tempCoeffs[$i] ?? '') }}"></td>
                                                        <td><input type="text" class="form-control" name="capacitor_drift[]" value="{{ old('capacitor_drift.' . $i, $drifts[$i] ?? '') }}"></td>
                                                        @if($i == 0)
                                                ...
                                                @else
                                                <tr>
                                                    <td><input type="text" class="form-control" name="id[]" placeholder="Enter Data"></td>
                                                    <td><input type="text" class="form-control" name="temp_coeff[]" placeholder="Enter Data"></td>
                                                    <td><input type="text" class="form-control" name="capacitor_drift[]" placeholder="Enter Data"></td>
                                                    <td><button type="button" class="btn btn-success" onclick="addSpecRow()">Add More</button></td>
                                                </tr>
                                                @endif
                                        ...
                                        <div class="col-12 text-end">
                                            <a href="{{ route('manage-specifications.index') }}" class="btn btn-danger px-4">Cancel</a>
       ...
       @include('components.backend.main-js')
        ...
        <script>
            ...
            function addSpecRow() {
                ...
                row.innerHTML = `
                    <td><input type="text" class="form-control" name="id[]" placeholder="Enter Data*" required></td>
                    <td><input type="text" class="form-control" name="temp_coeff[]" placeholder="Enter Data*" required></td>
                    <td><input type="text" class="form-control" name="capacitor_drift[]" placeholder="Enter Data*" required></td>
                    <td><button type="button" class="btn btn-danger" onclick="removeRow(this)">Remove</button></td>
                `;
            }
